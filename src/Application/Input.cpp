...
namespace Helix {
    ...
    static bool init_gamepad(int32_t index, Gamepad& gamepad) {
        ...
        if (pad) {
            HTRACE("Opened Joystick 0");
            HTRACE("Name: {}", SDL_GameControllerNameForIndex(index));
            ...
            HERROR("Couldn't open Joystick {}", index);
    ...
    InputBinding& InputBinding::set_handles(InputHandle action_map, InputHandle action) {
        ...
        HASSERT(action_map < 256);
        HASSERT(action < 16636);
