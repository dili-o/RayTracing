...
// for our time conversions).
		...
		f64 microseconds(i64 time) {
			return (f64)time;
		}
		f64 milliseconds(i64 time) {
			return (f64)time / 1000.0;
		}
		f64 seconds(i64 time) {
			return (f64)time / 1000000.0;
		}

		i64 from(i64 starting_time) {
			return now() - starting_time;
		}
		f64 from_microseconds(i64 starting_time) {
			return microseconds(from(starting_time));
		}
		f64 from_milliseconds(i64 starting_time) {
			return milliseconds(from(starting_time));
		}
		f64 from_seconds(i64 starting_time) {
		...
		f64 delta_milliseconds(i64 starting_time, i64 ending_time) {
