...
namespace Helix {
    ...
    Helix::FlatHashMap<u64, u32>   name_to_color;
    ...
    void GPUProfiler::update(GpuDevice& gpu) {
        ...
        for (u32 i = 0; i < active_timestamps; ++i) {
            ...
            u64 hashed_name = Helix::hash_calculate(timestamp.name);
            ...
            timestamp.color = Helix::Color::get_distinct_color(color_index);
        }
    ...
    void GPUProfiler::imgui_draw() {
...

        {
            ...
            for (u32 i = 0; i < max_frames; ++i) {
                ...
                frame_time = Helix::clamp(frame_time, 0.00001f, 1000.f);
                // Update timings
                new_average += frame_time;
                min_time = Helix::min(min_time, frame_time);
                max_time = Helix::max(max_time, frame_time);
