...
namespace Helix {
    ...
    // Graphics Data
    static Helix::TextureHandle g_font_texture;
    static Helix::PipelineHandle g_imgui_pipeline;
    static Helix::BufferHandle g_vb, g_ib;
    static Helix::BufferHandle g_ui_cb;
    static Helix::DescriptorSetLayoutHandle g_descriptor_set_layout;
    static Helix::DescriptorSetHandle g_ui_descriptor_set;  // Font descriptor set
    ...
    void ImGuiService::shutdown() {
        ...
        while (it.is_valid()) {
            Helix::ResourceHandle handle = g_texture_to_descriptor_set.get(it);
    ...
    void ImGuiService::render(Helix::CommandBuffer& commands, bool use_secondary) {
    ...
    void ImGuiService::remove_cached_texture(Helix::TextureHandle& texture) {
        FlatHashMapIterator it = g_texture_to_descriptor_set.find(texture.index);
        if (it.is_valid()) {

            // Destroy descriptor set
            Helix::DescriptorSetHandle descriptor_set{ g_texture_to_descriptor_set.get(it) };
    ...
    static Helix::Directory             directory;
    ...
    bool imgui_file_dialog_open( cstring button_name, cstring path, cstring extension ) {
        ...
        if ( opened && ImGui::Begin( "helix_imgui_file_dialog", &opened, ImGuiWindowFlags_AlwaysAutoResize ) ) {
            ...
            Helix::MemoryAllocator* allocator = Helix::memory_get_system_allocator();
            ...
            if ( strcmp( path, last_path ) != 0 ) {
                ...
                Helix::file_open_directory( path, &directory );
            ...
            if ( scan_folder ) {
                ...
                Helix::file_find_files_in_path( extension, directory.path, files, directories );
            }
            ...
            for ( size_t d = 0; d < directories.get_string_count(); ++d ) {
                ...
                if ( ImGui::Selectable( directory_name, selected, ImGuiSelectableFlags_AllowDoubleClick ) ) {
                    ...
                    if ( strcmp( directory_name, ".." ) == 0 ) {
                        Helix::file_parent_directory( &directory );
                    } else {
                        Helix::file_sub_directory( &directory, directory_name );
                    }
    ...
    bool imgui_path_dialog_open( cstring button_name, cstring path ) {
        ...
        if ( opened && ImGui::Begin( "helix_imgui_file_dialog", &opened, ImGuiWindowFlags_AlwaysAutoResize ) ) {
            ...
            if ( scan_folder ) {
                ...
                Helix::file_find_files_in_path( ".", directory.path, files, directories );
            }
