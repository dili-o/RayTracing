...
class Image {
public:
	Image(const char* filename, bool gpu = false) {
		i32 comp;
		stbi_set_flip_vertically_on_load(false);
		if (gpu) {
			u8* raw_bdata = stbi_load(filename, &image_width, &image_height, &comp, BYTES_PER_PIXEL);
			HASSERT_MSGS(raw_bdata, "Failed to load image: {}", filename);
			bdata.reset(raw_bdata);  // Take ownership
...
private:
	void convert_to_bytes() {
		i32 total_bytes = image_width * image_height * BYTES_PER_PIXEL;
